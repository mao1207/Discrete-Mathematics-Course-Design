...
public:
	...
	void printHaffmanCode(string& code);	//打印哈夫曼编码
	void printHaffmanCode(string& code, haffmanTreeNode<Type>* root);	//递归打印哈夫曼编码
	friend class haffmanTreeNode<Type>;
	friend class minHeap<haffmanTreeNode<Type>*>;
};
...
//打印哈夫曼编码
template <class Type>
void haffmanTree<Type>::printHaffmanCode(string& code) {
	printHaffmanCode(code, elems[size - 1]);
}

//递归打印哈夫曼编码
template <class Type>
void haffmanTree<Type>::printHaffmanCode(string &code, haffmanTreeNode<Type>* root) {
	if (root->lChild == NULL && root->rChild == NULL) {
		...
		code += "0";
		printHaffmanCode(code, root->lChild);
		code = codeBefore;
		code += "1";
		printHaffmanCode(code, root->rChild);
	}
}
